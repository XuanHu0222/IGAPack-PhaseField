function plotDispPhase2D(PHTelem,tdisp,sizeBasis,numberElements,geometry,controlPts,Mater, fenerg)
...
for indexPatch = 1:length(PHTelem)
    for i=1:length(PHTelem{indexPatch})
        if isempty(PHTelem{indexPatch}(i).children)
            ...
            sigmacoord((elementCounter-1)*4+1, 1) = stressTT{1,1};
            sigmacoord((elementCounter-1)*4+2, 1) = stressTT{1,2}';
            sigmacoord((elementCounter-1)*4+3, 1) = stressTT{2,2}';
            sigmacoord((elementCounter-1)*4+4, 1) = stressTT{2,1}';
